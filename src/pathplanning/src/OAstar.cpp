...
void OAstar::initOAstar(const OAstarConfig& config) {
    ...
    ROS_INFO("  safety_margin: %.2fm", o_config_.safety_margin);
...
bool OAstar::pathPlanning(cv::Point start_point, cv::Point target_point, std::vector<cv::Point>& path) {
    ...
    if (end_node == nullptr) {
        ROS_ERROR("OA* path planning failed! No path found after %d iterations", 
...
std::shared_ptr<ONode> OAstar::findPath(const cv::Point& start, const cv::Point& target) {
    ...
    ROS_WARN("OA* search failed: maximum iterations (%d) reached or no path found", 
             o_config_.max_iterations);
    return nullptr;
}
...
void OAstar::constrainedSmoothPath(std::vector<cv::Point>& path) {
    ...
    ROS_INFO("Applying constrained smoothing with %d iterations, weight: %.2f",
             o_config_.constrained_smoothing_iterations,
             o_config_.constrained_smoothing_weight);
...
void OAstar::bsplineSmooth(std::vector<cv::Point>& path) {
    ...
    while (iteration < max_iterations && !smoothing_success) {
        ...
        std::vector<std::pair<double, double>> spline_points = computeBSpline(control_points,
                                                                              o_config_.bspline_degree,
                                                                              o_config_.bspline_samples);
...
// 调整控制点以避免碰撞
std::vector<cv::Point> OAstar::adjustControlPointsForCollision(const std::vector<cv::Point>& control_points, int collision_index) {
...
// 计算B样条曲线
std::vector<std::pair<double, double>> OAstar::computeBSpline(const std::vector<cv::Point>& control_points, int degree, int samples) {
...
// 是否准备好规划
bool OAstar::isReadyForPlanning() const {
    return o_map_ready_ && o_start_ready_ && o_target_ready_;
}
